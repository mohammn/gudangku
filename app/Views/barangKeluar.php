        ...
        <div class="card">
            ...
            <div class="card-body">
                <form>
                    ...
                    <div class="form-group row">
                        <label for="inputPassword3" class="col-sm-2 col-form-label">Pengurangan Stok</label>
                        <div class="col-sm-10">
                            <input type="number" class="form-control" id="penguranganStok" name="penguranganStok">
                        </div>
                    </div>
                    <div class="form-group row">
...
<script>
    ...
    function muatData() {
        ...
        $.ajax({
            url: '<?= base_url() ?>/barangKeluar/muatData',
            method: 'post',
            dataType: 'json',
            success: function(data) {
                ...
                for (let b = data.length-1; b >= 0; b--) {
                    i++
                    tabel += "<tr><td>" + data[b].idBarang + "</td><td>" + data[b].namaBarang + "</td><td>" + data[b].penguranganStok + "</td><td>"+data[b].stokTerkini+"</td><td>"+data[b].satuan+"</td><td>"+data[b].keterangan+"</td></tr>"
                    if (i>4) {
                        break;
                    }
                }
                if (!tabel) {
                    tabel = '<td class="text-center" colspan="6">Pengurangan Barang Masih kosong :)</td>'
                }
                $("#tabelPenguranganStok").html(tabel)
    ...
    function barangTerpilih(){
        ...
        for (let i = 0; i < barang.options.length; i++) {
            if (idBarangTerpilih==barang.options[i].value) {
                $.ajax({
                    url: '<?= base_url() ?>/barangKeluar/barangById',
                    method: 'post',
    ...
    function tambah() {
        if ($("#penguranganStok").val() == 0 || $("#penguranganStok").val() == "") {
            $("#penguranganStok").focus();
        } else {
            $.ajax({
                type: 'POST',
                data: 'idBarang=' + $("#idBarang").val() + '&namaBarang=' + $("#namaBarang").val() + '&penguranganStok=' + $("#penguranganStok").val() + '&stokTerkini=' + $("#stokTerkini").val() + '&satuan=' + $("#satuan").val() + '&keterangan=' + $("#keterangan").val(),
